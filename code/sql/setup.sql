...
-- Standard PostgreSQL has no "CREATE USER IF NOT EXISTS" (unlike CREATE TABLE/INDEX) — the
-- previous version of this file used that syntax and would fail outright against real
-- Postgres. This is the idiomatic idempotent-role-creation pattern instead.
-- No password is set here deliberately — a real password belongs in deployment-specific
-- secrets/config, not committed schema DDL (this file used to hardcode one in plaintext).
-- Set it once after running this file:  ALTER ROLE store WITH PASSWORD '...';
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'store') THEN
        CREATE ROLE store WITH LOGIN;
    END IF;
END
$$;
...
-- ===== REQUIRED EXTENSIONS =====
-- Every extension is created up front. The previous version of this file created
-- pg_trgm/fuzzystrmatch/uuid-ossp near the very end, but the products table below (created
-- near the top) already indexes a column with gin_trgm_ops — that failed outright against
-- real Postgres ("operator class gin_trgm_ops does not exist") since the extension didn't
-- exist yet at the point it was first needed.
...
-- ===== SALES TRACKING =====
-- Indexes declared with separate CREATE INDEX statements below, matching every other table
-- in this file. The previous version used inline "INDEX idx_...(...)" clauses inside
-- CREATE TABLE, which is MySQL syntax — invalid PostgreSQL DDL, and this table failed to
-- create at all against real Postgres.
...
CREATE INDEX idx_period_sales_date ON store.period_sales(sale_date);
CREATE INDEX idx_period_sales_barcode ON store.period_sales(barcode);
...
-- (fuzzystrmatch/pg_trgm/uuid-ossp are created up front near the top of this file, not
-- here — see the "REQUIRED EXTENSIONS" section.)
